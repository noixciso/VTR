...
@Service
public class VirusDataServices {
    ...
    public void fetchVirusData() throws IOException, InterruptedException {
        ...
        for (CSVRecord record : records) {
            ...
            int laterCases = Integer.parseInt(record.get(record.size()-1));
            int prevDayCases = Integer.parseInt(record.get(record.size()-2));
            location.setLatestTotalCases(Integer.parseInt(record.get(record.size()-1)));
            location.setLatestTotalCases(Integer.parseInt(record.get(record.size()-2)));
            location.setLatestTotalCases(laterCases);
            location.setDelta(laterCases-prevDayCases);
